...
if(empty($_GET['search'])){
  ...
  if (!$result) {
    ...
    while ($row = $result->fetch_assoc()) {
      ...
      echo "<td class='text-center'><a href='?delid=".$row['id']."&name=del&page=".$page."&bookmarks=".$table."'><i class='text-danger glyphicon glyphicon-trash'></a></td>";
      echo "</tr>";}}
  else {
    ...
    if (!$result) {
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        echo "<td><a target='_blank' href='".$row['url']."'>" .$row['name']. "</td>";
        echo "<td>" .$row['url']. "</a></td>";
        echo "<td class='text-center'><a class='pull-center' href='?delid=".$row['id']."&name=del&search=".$_GET['search']."&bookmarks=".$table."'><i class='text-danger glyphicon glyphicon-trash'></a></td>";
        echo "</tr>";}}
?>
